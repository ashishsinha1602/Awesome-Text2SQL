...
- [schemagate](https://github.com/ashishsinha1602/schemagate)
  - schemagate decides which tables the model is allowed to see before it writes any SQL, taking the caller's identity at the selection step rather than filtering rows afterwards. Selection scores name, prose and body as separate BM25 channels and fuses them with optional dense vectors by reciprocal rank, with foreign-key closure and abstention on low-IDF queries. On Spider 1.0 pooled (876 tables, no database hint) it retrieves every table the gold answer needs 82.6% of the time at top 10; on Spider 2.0-lite, 79.8% at top 10 over all 247 usable questions; end-to-end execution accuracy on BIRD dev is 68.0% (102/150, seeded sample). Works over SQLAlchemy and ships an MCP server. Apache-2.0, Python 3.9+.
